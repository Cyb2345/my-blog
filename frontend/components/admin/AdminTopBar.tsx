...
function UserAvatar({
  ...
  user: Pick<AdminUser, "username" | "nickname" | "avatar"> | null;
}) {
  const label = (user?.nickname || user?.username || "A")
    .slice(0, 1)
    .toUpperCase();
  return user?.avatar ? (
    <img
...
export function AdminTopBar({
  ...
  const unreadItems = useMemo(
    () =>
      comments.filter(
        (item) => item.status === "pending" && !readIds.includes(item.id),
      ),
    [comments, readIds],
  );
  ...
  useEffect(() => {
    adminRequest<AdminUser>("/auth/me")
      .then(setUser)
      .catch(() => setUser(null));
    loadNotifications();
    const storedReadIds = window.localStorage.getItem(
      "admin_notification_read_comment_ids",
    );
    if (storedReadIds) {
      try {
        const values = JSON.parse(storedReadIds);
        if (Array.isArray(values))
          setReadIds(values.filter((value) => Number.isInteger(value)));
      } catch {
    ...
    try {
      ...
      window.setTimeout(() => {
        document.documentElement.classList.remove(
          "admin-fullscreen-transitioning",
        );
    ...
    >
      <div className="flex min-w-0 items-center gap-1">
        <div className="md:hidden">
          <IconButton
            label={mobileSidebarOpen ? t("关闭") : t("展开侧边栏")}
        ...
        {settings.showCollapse ? (
          <div className="hidden md:block">
            <IconButton
              label={sidebarCollapsed ? t("展开侧边栏") : t("收起侧边栏")}
        ...
        <div className="relative">
          <IconButton
            label={t("快捷页面")}
            active={openPanel === "quick"}
            onClick={() =>
              setOpenPanel((current) => (current === "quick" ? null : "quick"))
            }
          >
          ...
          <Popover
            ...
            className="left-0 w-[min(620px,calc(100vw-2rem))] p-4"
          >
            <div className="grid gap-5 sm:grid-cols-2">
              {quickGroups.map((group) => (
                <section key={group.label}>
                  <h3 className="mb-2 text-xs font-black uppercase tracking-wide text-muted-foreground dark:text-[var(--text-muted)]">
                    {t(group.label)}
                  </h3>
                    ...
                    {group.items.map((item) => {
                      ...
                      return item.external ? (
                        <Link
                          key={item.href}
                          href={item.href}
                          className="flex min-h-12 items-center gap-3 rounded-md px-3 hover:bg-muted dark:hover:bg-[var(--hover)]"
                        >
                          <span className="grid h-9 w-9 place-items-center rounded-md bg-[color-mix(in_srgb,var(--admin-primary)_12%,transparent)] text-[var(--admin-primary)]">
                        ...
                        <button
                          ...
                          className="flex min-h-12 items-center gap-3 rounded-md px-3 text-left hover:bg-muted dark:hover:bg-[var(--hover)]"
                        >
                          <span className="grid h-9 w-9 place-items-center rounded-md bg-[color-mix(in_srgb,var(--admin-primary)_12%,transparent)] text-[var(--admin-primary)]">
                            <ItemIcon className="h-4 w-4" />
                          </span>
              ...
              <span
                ...
                className={cn(
                  ...
                  index === breadcrumb.length - 1 &&
                    "text-foreground dark:text-[var(--text)]",
                )}
              >
                {index ? " / " : ""}
                {t(item)}
      ...
      <div className="ml-auto flex shrink-0 items-center gap-0.5 rounded-full border border-border bg-[color-mix(in_srgb,var(--admin-surface-muted)_82%,transparent)] px-1.5 py-1 shadow-sm">
        ...
        <div className="relative">
          ...
          <Popover
            ...
            className="right-0 w-[min(390px,calc(100vw-1rem))] overflow-hidden"
          >
            <div className="flex items-center justify-between border-b border-border px-4 py-3 dark:border-[var(--border-soft)]">
              <div>
                <h3 className="font-black">{t("消息通知")}</h3>
                <p className="mt-0.5 text-xs font-bold text-muted-foreground dark:text-[var(--text-muted)]">
                  {locale === "en-US"
              ...
              <button
                ...
                className="inline-flex items-center gap-1 text-xs font-black text-[var(--admin-primary)] disabled:opacity-40"
              >
            ...
            <div className="max-h-[420px] overflow-y-auto p-2">
              {notificationItems.length ? (
                notificationItems.map((item) => {
                  const unread =
                    item.status === "pending" && !readIds.includes(item.id);
                  return (
                    <button
                      key={item.id}
                      type="button"
                      ...
                      className="flex w-full items-start gap-3 rounded-md p-3 text-left hover:bg-muted dark:hover:bg-[var(--hover)]"
                    >
                      ...
                      <span className="min-w-0 flex-1">
                        ...
                        <span className="mt-1 block truncate text-xs font-bold text-muted-foreground dark:text-[var(--text-muted)]">
                          {item.content}
                        </span>
                        <span className="mt-1 block text-[11px] font-semibold text-muted-foreground dark:text-[var(--text-muted)]">
                ...
                <div className="grid min-h-32 place-items-center text-sm font-bold text-muted-foreground dark:text-[var(--text-muted)]">
                  {t("暂无通知")}
                </div>
              )}
            </div>
            <button
              type="button"
              onClick={() => navigate("/admin/site/messages")}
              className="min-h-11 w-full border-t border-border text-sm font-black text-[var(--admin-primary)] dark:border-[var(--border-soft)]"
            >
              {t("查看全部")}
            </button>
          </Popover>
        ...
        {settings.showLanguage ? (
          <div className="relative hidden sm:block">
            ...
            <Popover
              ...
              className="right-0 w-40 p-1"
            >
              {(
                [
                  ["zh-CN", "中文"],
                  ["en-US", "English"],
                ] as Array<[AdminLocale, string]>
              ).map(([value, label]) => (
                <button
                  key={value}
                  type="button"
                  ...
                  className={cn(
                    ...
                    locale === value
                      ...
                      : "hover:bg-muted dark:hover:bg-[var(--hover)]",
                  )}
                >
        ...
        <div className="relative">
          ...
          <Popover
            open={openPanel === "user"}
            className="right-0 w-64 overflow-hidden"
          >
            <div className="flex items-center gap-3 border-b border-border p-4 dark:border-[var(--border-soft)]">
              ...
              <span className="min-w-0">
                ...
                <span className="mt-0.5 block truncate text-xs font-bold text-muted-foreground dark:text-[var(--text-muted)]">
            ...
            <div className="p-1.5">
              <button
                ...
                className="flex min-h-10 w-full items-center gap-3 rounded-md px-3 text-sm font-bold hover:bg-muted dark:hover:bg-[var(--hover)]"
              >
              ...
              <button
                type="button"
                onClick={logout}
                className="flex min-h-10 w-full items-center gap-3 rounded-md px-3 text-sm font-bold text-[var(--danger)] hover:bg-destructive/10 dark:hover:bg-destructive/100/10"
              >
