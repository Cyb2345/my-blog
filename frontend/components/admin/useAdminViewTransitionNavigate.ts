...
function getStoredPageTransition(): AdminPageTransition {
  ...
  if (pageTransitionValues.has(stored as AdminPageTransition)) return stored as AdminPageTransition;

  const root = document.documentElement;
  const datasetValue = root.dataset.pageTransition ?? root.dataset.adminTransition;
  if (pageTransitionValues.has(datasetValue as AdminPageTransition)) return datasetValue as AdminPageTransition;
