...
function Thumbnail({ src, label }: { src?: string; label: string }) {
  ...
  if (!src || failed) {
    return (
      <span className="grid h-10 w-16 shrink-0 place-items-center rounded-lg bg-muted text-xs font-black text-muted-foreground ring-1 ring-border">
        无图
      </span>
    );
  }
  ...
  return (
    <div ref={rootRef} className={cn("relative", className)}>
      ...
      <button
        ...
        className={cn(
          "interactive flex min-h-10 w-full items-center gap-3 rounded-lg border border-input bg-background px-3 py-2 text-left text-sm font-bold text-foreground outline-none ring-[var(--admin-focus-ring)] hover:border-primary disabled:cursor-not-allowed disabled:opacity-60",
          open && "border-primary ring-4",
        )}
      ...
      {open ? (
        <div
          className={cn(
            "absolute left-0 right-0 top-[calc(100%+0.4rem)] z-[60] overflow-hidden rounded-xl border border-border bg-popover shadow-[var(--shadow-popover)]",
            panelClassName,
          )}
        >
          {searchable ? (
            <div className="border-b border-border p-2">
              <input
                ...
                placeholder="搜索"
                className="min-h-9 w-full rounded-lg border border-input bg-background px-3 text-sm font-semibold text-foreground outline-none focus:ring-4 focus:ring-[var(--admin-focus-ring)]"
              />
            </div>
          ) : null}
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      active
                        ...
                        : "text-foreground/72 hover:bg-muted",
                    )}
                  >
                    {option.thumbnail ? (
