...
export function PostEditor({ postId }: Props) {
  ...
  const [articleUploadProgress, setArticleUploadProgress] =
    useState<UploadProgressItem | null>(null);
  ...
  useEffect(() => {
    Promise.all([
      ...
      postId
        ? adminRequest<Post>(`/admin/posts/${postId}`)
  ...
  return (
    <Card className="motion-panel">
      <CardContent className="p-5">
        ...
        >
          {error ? (
            <p className="motion-notice rounded-md bg-destructive/10 px-3 py-2 text-sm font-bold text-destructive">
              {error}
            </p>
          ) : null}
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border bg-card px-3 py-2">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <Button
                    ...
                    size="sm"
                    onClick={() =>
                      setContentDraft((value) => normalizeYuqueMarkdown(value))
                    }
