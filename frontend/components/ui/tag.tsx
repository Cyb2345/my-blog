...
const variants: Record<TagVariant, string> = {
  neutral:
    "bg-[color-mix(in_srgb,var(--foreground)_8%,transparent)] text-muted-foreground ring-[color-mix(in_srgb,var(--foreground)_14%,transparent)]",
  primary:
    "bg-[color-mix(in_srgb,var(--primary)_12%,transparent)] text-primary ring-[color-mix(in_srgb,var(--primary)_24%,transparent)]",
  success:
  ...
  danger:
    "bg-[color-mix(in_srgb,var(--destructive)_14%,transparent)] text-destructive ring-[color-mix(in_srgb,var(--destructive)_28%,transparent)]",
...
export function Tag({ className, variant = "neutral", ...props }: TagProps) {
  return (
    <span
      className={cn(
        "inline-flex max-w-full items-center gap-1 rounded-md px-2 py-1 text-xs font-black ring-1",
        variants[variant],
        className,
      )}
      {...props}
