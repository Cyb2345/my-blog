  ...
  return (
    <>
      <div className="mb-6">
        ...
        <h1 className="mt-2 text-2xl font-black text-ink">标签管理</h1>
      </div>
      ...
      <div className="grid gap-5 lg:grid-cols-[360px_1fr]">
        <form key={editing?.id ?? "new"} onSubmit={handleSubmit} className="motion-surface grid gap-4 rounded-lg border border-ink/10 bg-white p-5 shadow-sm">
          <h2 className="text-lg font-black text-ink">{editing ? "编辑标签" : "新增标签"}</h2>
          <AdminField label="名称">
          ...
          <div className="flex gap-2">
            <Button type="submit" disabled={saving}>{editing ? "保存修改" : "新增标签"}</Button>
            {editing ? (
