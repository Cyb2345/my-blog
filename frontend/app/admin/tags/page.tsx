...
export default function AdminTagsPage() {
  ...
  async function load() {
    try {
      setItems(await adminRequest<Tag[]>("/admin/tags"));
    } catch (err) {
  ...
  async function remove(item: Tag) {
    ...
    setNotice("");
    try {
      await adminRequest(`/admin/tags/${item.id}`, { method: "DELETE" });
      await load();
      setNotice("标签已删除，列表已刷新。");
    } catch (err) {
  ...
  return (
    <>
      <div className="mb-6">
        <p className="text-sm font-bold text-ocean">Tags</p>
        <h1 className="mt-2 text-2xl font-black text-ink dark:text-slate-100">标签管理</h1>
      </div>
      ...
      <section className="motion-surface overflow-hidden rounded-lg border border-ink/10 bg-white shadow-sm dark:border-white/10 dark:bg-slate-900">
        <div className="flex items-center justify-between gap-3 border-b border-ink/10 p-4 dark:border-white/10">
          <Button type="button" onClick={() => setEditing({ id: 0, name: "", slug: "", description: "", post_count: 0, created_at: "", updated_at: "" })}>
      ...
      <AdminModal open={Boolean(editing)} title={editing?.id ? "编辑标签" : "新增标签"} size="sm" onClose={closeModal}>
        {editing ? (
