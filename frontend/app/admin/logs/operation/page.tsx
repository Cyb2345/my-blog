...
export default function AdminOperationLogsPage() {
  ...
  async function load(
    ...
    try {
      ...
      const data = await adminRequest<Paginated<OperationLog>>(
        `/admin/logs/operation?${query.toString()}`,
      );
      setItems(data.items);
      setPage(data.page);
      setPages(data.pages);
  ...
  async function confirmDelete() {
    ...
    setDeleteError("");
    try {
      await adminRequest(`/admin/logs/operation/${deleteItem.id}`, {
        method: "DELETE",
      });
      setDeleteItem(null);
      await load(page);
  ...
  const columns = useMemo<Array<AdminDataTableColumn<OperationLog>>>(
    () => [
      ...
      },
      {
        ...
        render: (item) => (
          <StatusTag
            status={item.request_method}
            label={item.request_method}
            map={{
              POST: { label: "POST", variant: "warning" },
              PUT: { label: "PUT", variant: "info" },
              DELETE: { label: "DELETE", variant: "danger" },
              PATCH: { label: "PATCH", variant: "primary" },
              GET: { label: "GET", variant: "success" },
            }}
          />
        ),
      },
      ...
      { key: "ip", title: "IP", width: 140, render: (item) => item.ip || "-" },
      {
        ...
        render: (item) => (
          <StatusTag
            status="duration"
            label={`${item.duration_ms} ms`}
            map={{
              duration: { label: `${item.duration_ms} ms`, variant: "info" },
            }}
          />
        ),
      },
      {
        ...
        title: "状态码",
        width: 100,
        render: (item) => item.response_code || "-",
      },
      {
        key: "createdAt",
        title: "创建时间",
            ...
            actions={[
              {
                key: "detail",
                label: "详情",
                icon: <Eye className={rowActionIconClass} />,
      ...
      >
        ...
        <AdminField label="请求方式">
          <CustomSelect
            value={method}
            onChange={setMethod}
            options={["", "POST", "PUT", "DELETE", "PATCH"].map((value) => ({
              label: value || "全部",
              value,
            }))}
          />
        ...
        pagination={
          <Pagination
            ...
            onPageSizeChange={(nextSize) => {
              ...
              void load(1, username, method, nextSize);
            }}
          />
        }
      />
        ...
        {detail ? (
          <div className="grid gap-3 text-sm">
            {[
              ...
              [
                "请求参数",
                detail.request_body || "未记录请求体，避免敏感字段入库",
              ],
            ].map(([label, value]) => (
