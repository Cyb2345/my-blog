...
export default function AdminAccessLogsPage() {
  ...
  const [tableSettings, setTableSettings] = useTableSettings(accessLogTableSettingsKey, defaultAccessLogTableSettings, accessLogColumnOptions);
  ...
  async function load(currentPage = pageNumber, currentPageSize = pageSize, currentFilters = appliedFilters) {
    ...
    setError("");
    try {
      const params = new URLSearchParams({ page: String(currentPage), page_size: String(currentPageSize) });
  ...
  function toggleCurrentPage(checked: boolean) {
    setSelectedIds(checked ? new Set(pageData.items.map((item) => item.id)) : new Set());
  }
  ...
  async function confirmDelete() {
    ...
    try {
      if (deleteState.type === "single" && ids.length === 1) {
        ...
        await adminRequest("/admin/logs/access/batch-delete", { method: "POST", body: JSON.stringify({ ids }) });
      }
      setDeleteState(null);
  ...
  const columns = useMemo<Array<AdminDataTableColumn<AccessLog>>>(
    () => [
      { key: "ip", title: "访问 IP", width: 180, hidden: !tableSettings.visibleColumns.includes("ip"), render: (item) => <span className="font-black text-foreground">{item.ip || "-"}</span> },
      { key: "location", title: "IP 归属地", minWidth: 240, ellipsis: true, hidden: !tableSettings.visibleColumns.includes("location"), render: (item) => item.ip_location || "-" },
      { key: "browser", title: "浏览器", width: 160, hidden: !tableSettings.visibleColumns.includes("browser"), render: (item) => item.browser || "Unknown" },
      { key: "os", title: "操作系统", width: 160, ellipsis: true, hidden: !tableSettings.visibleColumns.includes("os"), render: (item) => item.os || "Unknown" },
      { key: "createdAt", title: "访问时间", width: 210, hidden: !tableSettings.visibleColumns.includes("createdAt"), render: (item) => formatDateTime(item.created_at) },
      {
        key: "actions",
        title: "操作",
        width: 120,
        ...
        render: (item) => <RowActions actions={[{ key: "delete", label: "删除", icon: <Trash2 className={rowActionIconClass} />, variant: "delete", onClick: () => openSingleDelete(item) }]} />,
      },
    ],
  ...
  return (
    <AdminPage title="访问日志" description="查看前台访问来源、客户端信息并支持批量清理。">
      {error ? <Notice variant="error">{error}</Notice> : null}
      {notice ? <Notice variant="success">{notice}</Notice> : null}

      <AdminSearchForm onSubmit={handleQuery} loading={loading} actions={
        <>
          <Button type="submit" disabled={loading}><Search className="size-4" aria-hidden="true" />查询</Button>
          <Button type="button" variant="ghost" onClick={handleReset} disabled={loading}><RotateCcw className="size-4" aria-hidden="true" />重置</Button>
        </>
      }>
        <Input label="IP 地址" value={filters.ip} onChange={(event) => setFilters((current) => ({ ...current, ip: event.target.value }))} placeholder="请输入 IP 地址" />
        <Input label="IP 归属地" value={filters.location} onChange={(event) => setFilters((current) => ({ ...current, location: event.target.value }))} placeholder="请输入 IP 归属地" />
        <Input label="浏览器" value={filters.browser} onChange={(event) => setFilters((current) => ({ ...current, browser: event.target.value }))} placeholder="请输入浏览器" />
        <Input label="操作系统" value={filters.os} onChange={(event) => setFilters((current) => ({ ...current, os: event.target.value }))} placeholder="请输入操作系统" />
        <AdminField label="开始时间">
          <DateTimePicker value={filters.start_time} onChange={(value) => setFilters((current) => ({ ...current, start_time: value }))} placeholder="请选择开始时间" disabled={loading} />
        </AdminField>
        <AdminField label="结束时间">
          <DateTimePicker value={filters.end_time} onChange={(value) => setFilters((current) => ({ ...current, end_time: value }))} placeholder="请选择结束时间" disabled={loading} />
        </AdminField>
      </AdminSearchForm>
      ...
      <AdminDataTable
        ...
        toolbar={
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <AdminTableToolbar settings={tableSettings} onSettingsChange={setTableSettings} columns={accessLogColumnOptions} onRefresh={() => void load(pageData.page, pageSize, appliedFilters)} refreshing={loading} enableRefresh enableDensity enableStyle />
          </div>
        }
      ...
      <DeleteConfirmDialog open={Boolean(deleteState)} description={deleteDescription()} error={deleteError} loading={deleting} onClose={closeDeleteDialog} onConfirm={() => void confirmDelete()} />
    </AdminPage>
  );
}
