...
function FormSection({ title, children }: { title: string; children: ReactNode }) {
  return (
    <section className="grid gap-4 rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-muted)] p-4 md:grid-cols-2">
      <h3 className="font-black text-[var(--color-text)] md:col-span-2">{title}</h3>
      {children}
    </section>
  );
}
...
export default function AdminFileConfigPage() {
  ...
  const [settings, setSettings] = useTableSettings("admin-table-settings:files-config", defaultSettings, configColumnOptions);
    ...
    () => [
      {
        ...
        render: (config) => <span className="font-black text-[var(--color-text)]">{config.name}</span>,
      },
      {
        key: "storage",
        title: "存储器",
      ...
      },
      {
        ...
        render: (config) => <StatusTag status={config.is_primary} label={config.is_primary ? "是" : "否"} />,
      },
      {
        key: "status",
        title: "状态",
        ...
        render: (config) => <StatusTag status={config.status} map={statusMap} />,
      },
      {
        key: "publicUrl",
        title: "访问地址",
      ...
      },
      {
        ...
        render: (config) => <span className="text-[var(--color-text-muted)]">{formatDate(config.created_at)}</span>,
      },
      {
        key: "actions",
        title: "操作",
        ...
        render: (config) => (
          <RowActions
            actions={[
              { key: "edit", label: "编辑", icon: <Edit className={rowActionIconClass} aria-hidden="true" />, variant: "edit", onClick: () => openModal({ mode: "edit", item: config }) },
              { key: "test", label: "检查配置", icon: <CheckCircle2 className={rowActionIconClass} aria-hidden="true" />, variant: "neutral", onClick: () => void testConfig(config) },
              ...(!config.is_primary
                ? [{ key: "primary", label: "设为主配置", icon: <ShieldCheck className={rowActionIconClass} aria-hidden="true" />, variant: "warning" as const, onClick: () => void setPrimary(config) }]
                : []),
              { key: "delete", label: config.is_primary ? "主配置不可删除" : "删除", icon: <Trash2 className={rowActionIconClass} aria-hidden="true" />, variant: "delete", disabled: config.is_primary, onClick: () => setDeleteItem(config) },
            ]}
          />
        ),
      },
  ...
  return (
    <AdminPage title="文件配置" description="管理本地、Cloudflare R2 和 S3 Compatible 存储配置。">
      ...
      <AdminModal
        ...
        footer={
          <>
            ...
            <Button type="submit" form="file-config-form" loading={saving}>确定</Button>
          </>
        }
      >
        <form id="file-config-form" key={modal?.item?.id ?? "new"} onSubmit={saveConfig} className="grid gap-5">
          <ModalError message={modalError} />
          <FormSection title="基础信息">
            <Input name="name" label="配置名称" required defaultValue={modal?.item?.name ?? ""} />
            <Select
              label="存储器类型"
              value={storageType}
              onChange={(event) => setStorageType(event.target.value as FileStorageConfig["storage_type"])}
              options={[
            ...
            <div className="self-end">
              <Checkbox name="is_primary" label="设为主存储配置" defaultChecked={modal?.item?.is_primary ?? !configs.length} />
            </div>
          </FormSection>

          {storageType === "local" ? (
            <FormSection title="本地存储配置">
              <Input name="local_path" label="存储路径" required defaultValue={modal?.item?.local_path ?? "/app/uploads"} placeholder="/app/uploads" />
              <Input name="access_path" label="访问路径" required defaultValue={modal?.item?.access_path ?? "/uploads"} placeholder="/uploads" />
              <Input name="public_base_url" label="自定义域名" defaultValue={modal?.item?.public_base_url ?? ""} placeholder="https://static.example.com" />
              <Input name="base_path" label="基础路径" defaultValue={modal?.item?.base_path ?? "images"} placeholder="images" />
            </FormSection>
          ) : (
            <FormSection title={storageType === "r2" ? "Cloudflare R2 配置" : "S3 配置"}>
              <Input name="bucket" label="Bucket" required defaultValue={modal?.item?.bucket ?? ""} />
              <Input name="region" label="Region" defaultValue={modal?.item?.region ?? (storageType === "r2" ? "auto" : "")} />
              <Input name="endpoint" label="Endpoint" required defaultValue={modal?.item?.endpoint ?? ""} placeholder="https://" />
              <Input name="public_base_url" label="公开访问域名" required defaultValue={modal?.item?.public_base_url ?? ""} placeholder="https://" />
              <Input name="object_prefix" label="路径前缀" defaultValue={modal?.item?.object_prefix ?? "images"} />
              <Input name="access_key_id" label="Access Key ID" required={!modal?.item?.access_key_id} defaultValue={modal?.item?.access_key_id ?? ""} />
              <Input name="secret_access_key" label="Secret Access Key" type="password" required={!modal?.item?.secret_access_key} placeholder={modal?.item?.secret_access_key ? "已配置，留空不修改" : "请输入 Secret"} />
            </FormSection>
          )}

          <FormSection title="上传限制">
            <Input name="max_upload_size_mb" label="最大上传大小 MB" type="number" min={1} max={100} defaultValue={modal?.item?.max_upload_size_mb ?? 5} />
            <Input name="allowed_file_types" label="允许文件类型" defaultValue={modal?.item?.allowed_file_types ?? "image/jpeg,image/png,image/webp"} />
          </FormSection>
        </form>
      </AdminModal>
      ...
      <DeleteConfirmDialog
        ...
        description={deleteItem ? `确定删除文件配置「${deleteItem.name}」吗？` : "确定删除该配置吗？"}
        error={deleteError}
        loading={deleting}
