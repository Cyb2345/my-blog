    ...
    <section className="mx-auto max-w-6xl px-4 py-10">
      <div className="grid gap-6 lg:grid-cols-[17rem_minmax(0,1fr)_17rem]">
        ...
        <main className="min-w-0">
          <Link href="/docs" className="mb-6 inline-flex items-center gap-2 text-sm font-bold text-ocean dark:text-sky-300">
            <ArrowLeft className="h-4 w-4" aria-hidden="true" />
            返回知识库
          </Link>
          <MarkdownToc items={toc} mode="mobile" />
          <article className="motion-surface overflow-hidden rounded-lg border border-ink/10 bg-white/85 shadow-soft dark:border-white/10 dark:bg-slate-950/75">
            <div className="border-b border-ink/10 p-6 dark:border-white/10 md:p-9">
              <div className="mb-4 flex flex-wrap items-center gap-3 text-sm font-semibold text-ink/55 dark:text-slate-400">
              ...
              <h1 className="text-3xl font-black leading-tight text-ink dark:text-slate-100 md:text-5xl">{post.title}</h1>
              {post.summary ? <p className="mt-4 text-lg leading-8 text-ink/65 dark:text-slate-300">{post.summary}</p> : null}
            </div>
