...
function TaxonomyCard({ item, type }: { item: Category | Tag; type: "categories" | "tags" }) {
  ...
  return (
    <Link
      href={href}
      className="interactive-card rounded-lg border border-ink/10 bg-white/85 p-5 shadow-sm hover:shadow-soft dark:border-white/10 dark:bg-slate-950/75"
    >
      <div className="flex items-start gap-3">
        <span className="grid h-10 w-10 shrink-0 place-items-center rounded-md bg-ocean/10 text-ocean dark:bg-sky-400/10 dark:text-sky-300">
          <Icon className="h-5 w-5" aria-hidden="true" />
        </span>
        <div className="min-w-0">
          <h3 className="truncate text-lg font-black text-ink dark:text-slate-100">{item.name}</h3>
          <p className="mt-1 text-sm text-ink/55 dark:text-slate-400">
            {type === "categories" ? "分类" : "标签"} · {item.post_count ?? 0} 篇内容
          </p>
          {item.description ? (
            <p className="mt-3 line-clamp-2 text-sm leading-6 text-ink/65 dark:text-slate-300">{item.description}</p>
          ) : null}
        </div>
      </div>
    </Link>
...
function PostResultCard({ post }: { post: Post }) {
  return (
    <article className="interactive-card rounded-lg border border-ink/10 bg-white/85 p-5 shadow-sm hover:shadow-soft dark:border-white/10 dark:bg-slate-950/75">
      <div className="flex flex-wrap items-center gap-3 text-xs font-bold text-ink/50 dark:text-slate-500">
        <span className="inline-flex items-center gap-1">
          <CalendarDays className="h-3.5 w-3.5" aria-hidden="true" />
          {formatDate(post.published_at ?? post.created_at)}
        </span>
        {post.category ? (
          <Link href={`/categories/${post.category.slug}`} className="inline-flex items-center gap-1 hover:text-ocean dark:hover:text-sky-300">
      ...
      <Link href={`/posts/${post.slug}`}>
        <h3 className="mt-3 text-xl font-black leading-snug text-ink hover:text-ocean dark:text-slate-100 dark:hover:text-sky-300">{post.title}</h3>
      </Link>
      <p className="mt-3 line-clamp-2 text-sm leading-6 text-ink/65 dark:text-slate-300">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {post.tags.map((tag) => (
            <Link
              key={tag.id}
              href={`/tags/${tag.slug}`}
              className="interactive inline-flex items-center gap-1 rounded-md bg-ocean/10 px-2.5 py-1 text-xs font-semibold text-ocean hover:bg-ocean hover:text-white dark:bg-sky-400/10 dark:text-sky-300 dark:hover:bg-sky-400 dark:hover:text-slate-950"
            >
  ...
  return (
    <article className="interactive-card rounded-lg border border-ink/10 bg-white/85 p-5 shadow-sm hover:shadow-soft dark:border-white/10 dark:bg-slate-950/75">
      ...
      <Link href={`/docs/${post.slug}`}>
        <h3 className="mt-3 text-lg font-black leading-snug text-ink hover:text-ocean dark:text-slate-100 dark:hover:text-sky-300">{post.title}</h3>
      </Link>
      <p className="mt-3 line-clamp-2 text-sm leading-6 text-ink/65 dark:text-slate-300">
  ...
  return (
    <section className="mx-auto w-full max-w-5xl px-4 py-8 md:py-10">
      <div className="mb-6">
        ...
        <h1 className="mt-2 text-3xl font-black text-ink dark:text-slate-100">搜索</h1>
      </div>

      <SearchForm activeType={activeType} initialKeyword={keyword} />

      <div className="mt-5 flex flex-wrap items-center gap-2">
        <span className="text-sm font-bold text-ink/55 dark:text-slate-400">筛选：</span>
        {searchTypes.map((type) => {
          ...
          return (
            <Link
              ...
              className={
                active
                  ? "rounded-md bg-ink px-3 py-1.5 text-sm font-black text-white shadow-sm dark:bg-sky-400 dark:text-slate-950"
                  : "interactive rounded-md border border-ink/10 bg-white/70 px-3 py-1.5 text-sm font-bold text-ink/65 hover:border-ocean/30 hover:text-ocean dark:border-white/10 dark:bg-white/5 dark:text-slate-300 dark:hover:text-sky-300"
              }
            >
              {type.label}
            </Link>
        ...
        {!keyword ? (
          ...
          <div className="space-y-8">
            {showPosts && posts.items.length ? (
              <section>
                <h2 className="mb-4 text-lg font-black text-ink dark:text-slate-100">文章结果</h2>
            ...
            {showDocs && posts.items.length ? (
              <section>
                <h2 className="mb-4 text-lg font-black text-ink dark:text-slate-100">知识库文档</h2>
            ...
            {showCategories && categoryResults.length ? (
              <section>
                <h2 className="mb-4 text-lg font-black text-ink dark:text-slate-100">分类结果</h2>
                <div className="motion-list grid gap-4 md:grid-cols-2">
            ...
            {showTags && tagResults.length ? (
              <section>
                <h2 className="mb-4 text-lg font-black text-ink dark:text-slate-100">标签结果</h2>
                <div className="motion-list grid gap-4 md:grid-cols-2">
                  {tagResults.map((tag) => (
